...
library(pacman)
p_load(tidyverse, tidytext, stringr, lmerTest, lme4, dplyr, ggplot2, data.table)
```
...
data %>% filter(VISIT==1) %>% group_by(Diagnosis) %>%  summarize(Number=n(), Mean_age=mean(Age), Girls=sum(Gender=="F"))
```
...
null_curve <- lmer(CHI_MLU ~ VISIT + Diagnosis + VISIT2 + (1 | SUBJ) + (0 + VISIT | SUBJ), data=data)

#Specifying full model
model3 <- lmer(CHI_MLU ~ VISIT + Diagnosis + VISIT2 + VISIT*Diagnosis + VISIT2*Diagnosis + (1 | SUBJ) + (0 + VISIT | SUBJ), data=data)
summary(model3)

# Comparing the models
anova(null_curve,model3)
```
