...
```{r ex4, include = FALSE}
#Specifying full model
model_3 <- lmer(CHI_MLU ~ 1 + Diagnosis * (ExpressiveLangRaw1 + MullenRaw1) * (VISIT + VISIT2) + (1 + VISIT + VISIT2 | SUBJ), data = data)

anova(model2,model3)

# Excluding MullenRaw1
model4 <- lmer(CHI_MLU ~ 1 + Diagnosis * ExpressiveLangRaw1 * (VISIT +  VISIT2) + (1 + VISIT + VISIT2 | SUBJ), data = data)
summary(model4)
